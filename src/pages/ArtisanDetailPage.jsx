...
export default function ArtisanDetailPage() {
  ...
  const { data: avis = [] } = useArtisanAvis(id)
  const createDemande = useCreateDemande()
  const [showDemandeForm, setShowDemandeForm] = useState(false)
  const [description, setDescription] = useState('')

  if (isLoading) return <div className="flex justify-center py-24"><Spinner size="lg" /></div>
  if (!artisan)  return <div className="text-center py-24 text-gray-500">Artisan introuvable.</div>
  ...
  return (
    <main className="max-w-4xl mx-auto px-4 py-8">
      <Link to="/artisans" className="text-sm text-gray-500 hover:text-primary-500 mb-6 inline-block">
        ← Retour aux artisans
      </Link>
      ...
      <div className="grid md:grid-cols-3 gap-6">
        ...
        <div className="md:col-span-1">
          <div className="card p-6 text-center">
            {artisan.photo ? (
              <img src={artisan.photo} alt={artisan.prenom} className="w-24 h-24 rounded-2xl object-cover mx-auto mb-4" />
            ) : (
            ...
            <p className="text-primary-600 font-medium mt-1">{artisan.metier_label}</p>
            ...
            <div className="flex items-center justify-center gap-1.5 mt-3">
              ...
              <span className="text-sm text-gray-600">{artisan.quartier}, {artisan.ville}</span>
            </div>
            ...
            <div className="mt-5 flex flex-col gap-2">
              {artisan.whatsapp && (
                <a href={whatsappUrl} target="_blank" rel="noopener noreferrer" className="btn-whatsapp justify-center">
              ...
              {artisan.telephone && (
                <a href={`tel:${artisan.telephone}`} className="btn-secondary text-center">
                  📞 Appeler
                </a>
              )}
              {user && user.role !== 'artisan' && (
                <button onClick={() => setShowDemandeForm(true)} className="btn-primary">
                  Envoyer une demande
                </button>
              )}
            </div>
        ...
        <div className="md:col-span-2 flex flex-col gap-5">
          {artisan.bio && (
            <div className="card p-6">
              <h2 className="font-display font-semibold text-gray-900 mb-3">À propos</h2>
          ...
          {showDemandeForm && (
            <div className="card p-6 border-2 border-primary-200">
              <h2 className="font-display font-semibold text-gray-900 mb-3">Envoyer une demande</h2>
              <form onSubmit={handleDemande} className="flex flex-col gap-3">
                ...
                <div className="flex gap-2">
                  <button type="submit" className="btn-primary" disabled={createDemande.isLoading}>
                    {createDemande.isLoading ? 'Envoi...' : 'Envoyer'}
                  </button>
                  <button type="button" onClick={() => setShowDemandeForm(false)} className="btn-secondary">
                    Annuler
                  </button>
                </div>
              </form>
          ...
          <div className="card p-6">
            ...
            {avis.length === 0 ? (
              ...
              <div className="flex flex-col gap-4">
                {avis.map(a => (
                  <div key={a.id} className="border-b border-sand-100 pb-4 last:border-0 last:pb-0">
                    <div className="flex items-center justify-between mb-1">
                      <span className="font-medium text-sm text-gray-800">{a.client_nom}</span>
                    ...
                    {a.commentaire && <p className="text-sm text-gray-600 mt-1">{a.commentaire}</p>}
                    {a.reponse_artisan && (
                      <div className="mt-3 rounded-xl bg-sand-50 border border-sand-100 p-3">
                        <div className="flex items-center justify-between gap-3 mb-1">
                          <p className="text-xs font-medium text-gray-500">Reponse de l'artisan</p>
                        ...
                        <p className="text-sm text-gray-700">{a.reponse_artisan}</p>
                      </div>
                    )}
                  </div>
                ))}
