  ...
  return (
    <div className="card group p-5 flex flex-col gap-4">
      ...
      <div className="flex items-start gap-4">
        <div className="relative flex-shrink-0">
          {artisan.photo ? (
            ...
            <div className="w-16 h-16 rounded-2xl bg-primary-100 flex items-center justify-center text-primary-600 font-bold text-xl font-display">
              {artisan.prenom?.[0]}{artisan.nom?.[0]}
            </div>
          )}
          {artisan.disponible && (
            <span className="absolute -bottom-1 -right-1 w-4 h-4 bg-accent-500 border-2 border-white rounded-full" title="Disponible" />
          )}
        </div>
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-primary-600 font-medium text-sm">{artisan.metier_label}</p>
          <div className="flex items-center gap-1.5 mt-1">
            <span className="text-gray-400 text-xs">📍</span>
            <span className="text-xs text-gray-500 truncate">{artisan.quartier}, {artisan.ville}</span>
          </div>
        </div>
      </div>
      ...
      <div className="flex items-center justify-between">
        ...
        <span className="text-xs text-gray-400">{artisan.nb_avis || 0} avis</span>
        {artisan.disponible
      ...
      {/* Bio */}
      {artisan.bio && (
        <p className="text-sm text-gray-600 line-clamp-2 leading-relaxed">{artisan.bio}</p>
      )}
